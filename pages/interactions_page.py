...
class SortablePage(BasePage):
    ...
    @allure.step("Метод сортировки")
    def get_sortable_items(self, elements):
        item_list = self.elements_are_visible(elements)
        return [item.text for item in item_list]

    @allure.step("Метод для изменения списка элементов")
    ...
    @allure.step("Метод для изменения сетки элементов")
...
class SelectablePage(BasePage):
    ...
    @allure.step("Метод для клика нужного элемента")
    def click_selectable_item(self, elements):
        item_list = self.elements_are_visible(elements)
        random.sample(item_list, k=1)[0].click()

    @allure.step("Метод для выбора элемента из списка")
    ...
    @allure.step("Метод для выбора элемента из сетки")
...
class ResizablePage(BasePage):
    ...
    @allure.step("Метод для получения данных из высоты и ширины")
    def get_px_from_width_height(self, value_of_size):
        width = value_of_size.split(';')[0].split(':')[1].replace(' ', '')
        height = value_of_size.split(';')[1].split(':')[1].replace(' ', '')
        return width, height

    @allure.step("Метод для получения максимального и минимального размера")
    def get_max_min_size(self, element):
        size = self.element_is_visible(element)
        size_value = size.get_attribute('style')
        return size_value

    @allure.step("Метод изменения размера большого окна")
    ...
    @allure.step("метод изменения размера малого окна")
...
class DroppablePage(BasePage):
    ...
    @allure.step("Метод для Prevent propogation")
    def drop_prevent_propogation(self):
        self.element_is_visible(self.locators.PREVENT_TAB).click()
        drag_div = self.element_is_present(self.locators.DRAG_ME_PREVENT)
    ...
    @allure.step("Метод для revert draggable")
...
class DraggablePage(BasePage):
    ...
    @allure.step("Метод получения первоначальных позицией элементов")
    ...
    @allure.step("Метод для Simple drag")
    ...
    @allure.step("Метод движения элемента по Y")
